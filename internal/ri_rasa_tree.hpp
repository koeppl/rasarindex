...
public:
  ...
  rads_tree(){};
  rads_tree(std::vector<uint> &cycle, std::vector<std::pair<ulint, ulint>> &bounds, uint tree_num, std::vector<std::pair<uint, uint>> &tree_pointers){
  ...
  void constructor_helper(std::vector<uint> &cycle, std::vector<std::pair<ulint, ulint>> &bounds, size_t node, size_t begin, size_t end, uint tree_num, std::vector<std::pair<uint, uint>> &tree_pointers) {
    ...
    if(begin >= end) { // condition which creates the leaf nodes.
      ...
      tree_pointers.push_back(std::make_pair(tree_num, node));
      return;
    }
