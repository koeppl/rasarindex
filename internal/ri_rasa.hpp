...
static constexpr size_t MIN_PATH_SIZE = 1000; //@ lower bound on the number of nodes a rads-tree indexes
  ...
  // void helper_query(ulint &sa_j, ulint d, ulint run, rle_string_t &bwt, int_vector<> &pred_to_run, sparse_bv_type &pred, int_vector<> &sa, std::vector<ulint> &ssa) {
    ...
    while(d > 0) {
      ...
      if(trees_bv[pred_to_run[sa_jr]] && d != 1) {
        ...
        const std::tuple<ulint, ulint, ulint> sa_prime_d_cost = trees[std::get<1>(tree_info)].query(std::get<2>(tree_info), cost, d, std::get<0>(tree_info)); // tuple containing new sample run, distance travelled, and cost accumulated
